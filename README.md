...
Una aplicación web de flashcards interactiva para aprender inglés u otros idiomas. CardFlow AI propone rondas cortas de estudio: muestra una tarjeta, permite voltearla para ver el significado y pide una autoevaluación al estilo Tinder. Es una aplicación estática, sin servidor ni dependencias, preparada para publicarse gratis en [GitHub Pages](https://github.com/CONTERNICO845/CardFlow-AI).
...
2. Pégalo en ChatGPT, Claude o la IA que prefieras, seguido de tu lista de palabras o expresiones.
3. Copia únicamente la respuesta JSON de la IA.
4. Pégala en la pestaña **Pegar JSON** de CardFlow AI, revisa que aparezca el mensaje de tarjetas listas e inicia la ronda.

> Actúa como un generador de flashcards JSON. A continuación te proporcionaré una lista continua de palabras, frases cortas o expresiones en inglés (separadas por comas o espacios). Tu tarea es procesarlas, traducirlas al español de forma natural y estructurarlas estrictamente en formato de arreglo JSON válido con las claves 'front' (término en inglés) y 'back' (significado en español). No agregues texto adicional, solo devuelve el código JSON.
...
1. **Repetición espaciada:** guardar dificultad, fecha de próxima revisión y un algoritmo SM-2 o Leitner para priorizar tarjetas.
2. **Persistencia con localStorage:** conservar mazos, ajustes de sesión e historial de progreso directamente en el navegador.
3. **Síntesis de voz (TTS):** usar Web Speech API para pronunciar el contenido de `front`, con selector de voz, idioma y velocidad.
